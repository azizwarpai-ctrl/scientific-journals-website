...
| A3 | Real "Recent Activity" numbers | [repo] | A0 | todo | Replace the three `Math.floor(* fraction)` lines at `app/admin/analytics/page.tsx:174-198` with `UserEvent` aggregates. No UI redesign. Unit test on the aggregate query. |
| A4 | Real "System Health" probes | [repo] | A0 | todo | Wire `prisma.$queryRaw` ping + `ojsHealthCheck()` into `app/admin/analytics/page.tsx:153-172`. Render colours from the probe result, not from a constant. |
