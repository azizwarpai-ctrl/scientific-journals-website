...
// --- CONFIGURATION ---
// Check environment vars first, fallback to config.inc.php [digitopub] base_url
$envRawUrl = getenv('DIGITOPUB_BASE_URL') ?: ($_ENV['DIGITOPUB_BASE_URL'] ?? Config::getVar('digitopub', 'base_url'));
if (empty($envRawUrl)) {
    die("Error: DIGITOPUB_BASE_URL environment variable (or config.inc.php [digitopub] base_url) is not configured.");
}
...
// Ensure the redirect uses the correct base URL (e.g., if OJS is installed in a subdirectory like /ojs)
$ojsBaseUrl = rtrim($request->getBaseUrl(), '/');
// Strip any leading slashes from redirect to cleanly append it
